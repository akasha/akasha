  ...
  extends AbstractTest
{
    ...
    throws IOException
  {
    ...
    WebIDLWriter.writeNamespaceDefinition( writer, actual );
    writer.close();
    final String emittedIDL = writer.toString();
    final List<Definition> definitions = WebIDLModelParser.parse( createParser( emittedIDL ).definitions() );
    assertEquals( definitions.size(), 1 );
    assertTrue( definitions.get( 0 ) instanceof NamespaceDefinition );
    final NamespaceDefinition element = (NamespaceDefinition) definitions.get( 0 );
