...
* Change the way the java binding generates unions so that methods of the form `as[X]()` and `is[X]()` exist for every reference component type `X` that is part of the union.
