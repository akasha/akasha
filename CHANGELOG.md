...
* Correct the externs for operations that returned typed `Iterator<X>` types. These were incorrectly represented as `<ArrayX>` in the closure externs but were correctly represented as `JsIterator<X>` in jsinterop annotated classes.
* Generate closure externs for operations and properties synthesized when a `maplike` member is present on an interface type. This change partially addresses #28.
